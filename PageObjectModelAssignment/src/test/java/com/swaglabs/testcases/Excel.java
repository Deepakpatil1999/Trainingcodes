	...
	public ArrayList<String> getData() throws IOException {
		...
		for (int i = 0; i < sheets; i++) {
			if (workbook.getSheetAt(i).getSheetName().equalsIgnoreCase("Sheet2")) {
				...
				while (rows.hasNext()) {
					Row r = rows.next();
					if (r.getCell(coloum).getStringCellValue().equalsIgnoreCase("SwagLabLoginPage")) {
						...
						while (ce.hasNext()) {
							...
							/*
							 * if(r.getCell(coloum).getStringCellValue().equalsIgnoreCase(
							 * "SwagLabInformationPage")) { Iterator<Cell> ce1 = r.cellIterator();
							 * 
							 * while (ce1.hasNext()) { arr.add(ce1.next().getStringCellValue());
							 */
